...
class LoginService extends Network {
  Future<LoginResponse> login(LoginRequest request) async {
    ...
    if (response.statusCode == 200) {
      ...
      final String? fullName = listCookies?[2].split(';').first.split('=').last;
      final String? userId = listCookies?[3].split(';').first.split('=').last;
      final String? userImage =
          listCookies?[4].split(';').first.split('=').last;
      if (sid != null) {
        await storeData(sid, expiredDate, fullName, userId, userImage);
...
Future<void> storeData(String sid, DateTime? expiredDate, String? fullName,
